import EventChannel from '@tronmask/lib/EventChannel';
...
import { CONTRACT_ADDRESS, SIDE_CHAIN_ID, NODE, SIDE_CHAIN_ID_TEST } from '@tronmask/lib/constants';
...
const pageHook = {
    ...
    init() {
        ...
        this.request('init').then(({ address, node, name, type, phishingList, connectNode = false }) => {
            ...
            logger.info('TronMask initiated');
    ...
    _bindTronWeb() {
        if (window.tronWeb !== undefined) {
            logger.warn('TronWeb is already initiated. TronMask will overwrite the current instance');
        }
        ...
        ['setPrivateKey', 'setAddress', 'setFullNode', 'setSolidityNode', 'setEventServer'].forEach(method => {
            tronWeb[method] = () => new Error('TronMask has disabled this method');
            sunWeb.mainchain[method] = () => new Error('TronMask has disabled this method');
            sunWeb.sidechain[method] = () => new Error('TronMask has disabled this method');
        });

        ['setMainGatewayAddress', 'setSideGatewayAddress', 'setChainId'].forEach(method => {
            sunWeb[method] = () => new Error('TronMask has disabled this method');
        });
    ...
    setAddress({ address, name, type }) {
        // logger.info('TronMask: New address configured');
        if (!tronWeb.isAddress(address)) {
            tronWeb.defaultAddress = {
                hex: false,
                base58: false
            };
            sunWeb.mainchain.defaultAddress = {
                hex: false,
                base58: false
            };
            sunWeb.sidechain.defaultAddress = {
                hex: false,
                base58: false
            };
            tronWeb.ready = false;
